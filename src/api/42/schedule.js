...
import api42 from "../42Api.js";
import { postDM2User } from "../apiSlack.js";
import * as dbLocation from "../../DataBase/dbLocation.js";
	...
	loadLocations: (delay) => {
		...
		_schedule.scheduleJob(`*/${+delay} * * * * *`, async () => {
			try {
				...
				if (last == 0) dbLocation.deleteAllLocationTable();
				else dbLocation.deleteLocationTable(deleteTargets);
				const locationTable = total.reduce((acc, cur) => {
					if (cur.end_at == null) acc[cur.user.login] = cur.host;
					return acc;
				}, {});
				await dbLocation.replaceLocationStatus(locationTable);
				last = now;
				...
				for (let id in alarms)
					if (alarms[id].notify_slack_id)
						postDM2User(
							alarms[id].notify_slack_id,
							`${alarms[id].target_id} is online on ${alarms[id].host}`
						);
				await dbAlarm.deleteReservedAlarm(
					alarms.map((x) => x.alarm_id)
				);
			} catch (e) {
				console.error(e);
			}
