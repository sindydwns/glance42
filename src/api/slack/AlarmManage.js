...
export default (app) => {
	...
	app.view(
		...
		async ({ ack, body, view, client, logger }) => {
			const selectedUsersSlackId =
				view.state.values[view.blocks[0].block_id].selectAddAlarm
					.selected_users;
			const selectedUsersIntraId = await Promise.all(
				selectedUsersSlackId.map((x) => getUserNamebySlackId(client, x))
			);
			const intraId = await getClientIntraId(body, null, client);
			const duplicatedAlarm = await dbAlarm.selectDuplicatedAlarm(
				intraId,
	...
	app.view(
		...
		async ({ ack, body, view, client, logger }) => {
			await ack();
			const selectedAlarms = view.state.values[
