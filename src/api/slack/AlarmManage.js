...
export default (app) => {
	...
	app.view(
		...
		async ({ ack, body, view, client, logger }) => {
			const selectedUsersSlackId =
				view["state"]["values"][view.blocks[0].block_id][
					"selectAddAlarm"
				]["selected_users"];
			...
			const seekerId = await getClientIntraId(body, null, client);
			const duplicatedAlarm = await selectDuplicatedAlarm(
				seekerId,
				selectedUsersIntraId
			);

			if (duplicatedAlarm.length != 0) {
				const duplicatedAlarmStr = duplicatedAlarm
					.map((x) => `'${x.target_id}'`)
					.join(", ");
				await ack({
	...
	app.view(
		...
		async ({ ack, body, view, client, logger }) => {
			await ack();
			const deleteAlarms = view["state"]["values"][
				view.blocks[0].block_id
			]["selectDelAlarm"]["selected_options"].map((x) => x.value);
			const seekerId = await getClientIntraId(body, null, client);

			let msg = "";
			for (const targetId of deleteAlarms) {
