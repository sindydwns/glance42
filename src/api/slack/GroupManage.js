...
export default (app) => {
	...
	app.view(
		{ callback_id: "callbackAddGroup", type: "view_submission" },
		async ({ ack, body, view, client, logger }) => {
			const inputVal =
				view.state.values[view.blocks[0].block_id].writeAddGroupName
					.value;
			const intraId = await getClientIntraId(body, null, client);
	...
	app.view(
		{ callback_id: "callbackDelGroup", type: "view_submission" },
		async ({ ack, body, view, client, logger }) => {
			await ack();
			const inputVal =
				view.state.values[view.blocks[0].block_id].selectDelGroup
	...
	app.view(
		{ callback_id: "callbackModifyGroupName", type: "view_submission" },
		async ({ ack, body, view, client, logger }) => {
			await ack();
			const modyfyGroupId =
				view.state.values[view.blocks[0].block_id].selectModifyNameGroup
					.selected_option.value;
			const modyfyGroupName =
				view.state.values[view.blocks[1].block_id].writeModifyGroupName
					.value;
			const seekerId = await getClientIntraId(body, null, client);
			let msg = "";
			const result = await dbGroup.updateGroupName(
				modyfyGroupId,
				modyfyGroupName
			);
