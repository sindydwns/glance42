...
{	
	if (selectedType == "selectGroup" && locationInfo == null) {
		...
		return [...Blocks.SectionMrkdwn(`마지막 업데이트: ${timeStamp()}`),
...
export async function mainHomeView(intraId, selectedUsersFromWorkspace, msg) {
	...
	if (msg)
		messageBlock = Blocks.SectionMrkdwn(msg);

	return (HomeViewTemplete([
		...Blocks.Header("👀 염탐하기"),
		...Blocks.SectionMrkdwn("\n"),
		...Blocks.SectionSelect("염탐할 대상을 선택해주세요", "selectGlanceTarget", selectOptionList, initialSelect),
		...Blocks.SectionMrkdwn("\n"),
		...
		...Blocks.Divider(),
		...Blocks.SectionMrkdwn("\n"),
		...Blocks.SectionButton("_사용방법을 모르시겠나요? 여기를 참고하세요!_ 📚", {text:"Help", value:"Help"}, "goManualView"),
	]));
}
...
export async function notRegisteredHomeView(slackId) {
	return (HomeViewTemplete([
			...Blocks.Header("👋 환영합니다!"),
			...Blocks.SectionMrkdwn("Glance42를 처음 이용하시는군요!\
			...
			...Blocks.SectionMrkdwn("처음 서비스를 이용하기 전 간단한 42API 인증이 필요합니다."),
			...Blocks.LinkButton("'인증하기'를 통해 카뎃임을 인증해주세요. \n(인증은 최초 한번만 이루어집니다.)", 
...
export async function groupManageHomeView(intraId, msg) {
	...
	return (HomeViewTemplete([
		...
		...Blocks.ActionButtons([{text:"< back", value:"뒤로가기", actionId:"goMainView"}]),
		...Blocks.Divider(),
		...Blocks.SectionMrkdwn("\n"),
...
export async function memberManageHomeView(intraId, selectGroup, msg) {
	...
	if (selectGroup) {
		...
		return HomeViewTemplete([
			...
			...Blocks.ActionButtons([{text:"< back", value:"뒤로가기", actionId:"goGroupManageView"}]),
			...Blocks.Divider(),
			...Blocks.SectionMrkdwn("\n"),
			...Blocks.SectionSelect("멤버를 관리할 그룹을 선택해주세요", "selectGroupforMemberManage", groupList, selectGroup),
			...Blocks.SectionMrkdwn("\n"),
		...
		return HomeViewTemplete([
			...
			...Blocks.Divider(),
			...Blocks.SectionMrkdwn("\n\n"),
			...Blocks.SectionSelect("멤버를 관리할 그룹을 선택해주세요", "selectGroupforMemberManage", groupList, false),
		]);
}
...
export async function selectUserFromWorkspaceModalView() {
	return (ModalViewTemplete("워크스페이스에서 유저 선택", "callbackSelectUserFromWorkspace", ([
			Blocks.MultiUsersSelect("염탐할 유저를 선택해주세요.\n(물론, 카뎃만 선택 가능합니다!)",
...
export async function addGroupModalView() {
	return (ModalViewTemplete("그룹 추가", "callbackAddGroup", ([
			Blocks.TextInput("추가할 그룹명을 입력해주세요", "writeAddGroupName", "textInput-groupName"),
		])
	));
}
...
export async function modifyGroupNameModalView(seekerId) {
	...
	return (ModalViewTemplete("그룹 이름 변경", "callbackModifyGroupName", ([
			Blocks.SingleStaicSelect("이름을 변경할 그룹을 선택해주세요", "selectModifyNameGroup", groupList),
			Blocks.TextInput("변경할 그룹명을 입력해주세요", "writeModifyGroupName", "textInput-groupName")
		])
	));
}
...
export async function addAlarmModalView() {
	return (ModalViewTemplete("알람 추가", "callbackAddAlarm", ([
			Blocks.MultiUsersSelect("알람을 받을 유저를 선택해주세요\n(물론, 카뎃만 선택 가능합니다!)",
...
export async function delAlarmModalView(intraId) {
	...
	return (ModalViewTemplete("알람 삭제", "callbackDelAlarm", ([
			Blocks.MultiStaicSelect("삭제할 알람을 선택해주세요", "selectDelAlarm", alarmList)
		])
	));
}
...
export async function addMemberModalView() {
	return (ModalViewTemplete("멤버 추가", "callbackAddMember", ([
			Blocks.MultiUsersSelect("멤버로 추가할 유저를 선택해주세요\n(물론, 카뎃만 선택 가능합니다!)",
...
export async function delMemberModalView(groupId) {
	...
	return (ModalViewTemplete("멤버 삭제", "callbackDelMember", ([
			Blocks.MultiStaicSelect("그룹에서 삭제할 멤버를 선택해주세요", "selectDelMember", memberList)
		])
	));
}
