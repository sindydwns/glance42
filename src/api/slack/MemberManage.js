...
export default (app) => {
	...
	app.view(
		{ callback_id: "callbackAddMember", type: "view_submission" },
		async ({ ack, body, view, client, logger }) => {
			const selectedUsersSlackId =
				view.state.values[view.blocks[0].block_id].selectAddMember
					.selected_users;
			const selectedUsersIntraId = await Promise.all(
				selectedUsersSlackId.map((x) => getUserNamebySlackId(client, x))
			);
			const seekerId = await getClientIntraId(body, null, client);
			const selectedGroupId = await dbGroup.getSelectedGroupId(seekerId);
			const duplicatedGroupMember =
				await dbGroup.selectDuplicatedGroupMember(
					selectedGroupId,
					selectedUsersIntraId
				);

			if (duplicatedGroupMember.length != 0) {
				const duplicatedGroupMemberStr = duplicatedGroupMember
					.map((x) => `'${x.targetId}'`)
					.join(", ");
			...
			for (const targetId of selectedUsersIntraId) {
				const result = await dbGroup.insertMember(
					selectedGroupId,
					targetId
				);

				if (result) msg = "*성공적으로 추가되었습니다*";
			}
			try {
				const result = await client.views.publish({
					user_id: body.user.id,
	...
	app.view(
		{ callback_id: "callbackDelMember", type: "view_submission" },
		async ({ ack, body, view, client, logger }) => {
			await ack();
			const selectedMembers = view.state.values[
				view.blocks[0].block_id
			].selectDelMember.selected_options.map((x) => x.value);
			const seekerId = await getClientIntraId(body, null, client);
			const selectedGroupId = await dbGroup.getSelectedGroupId(seekerId);
			...
			for (const targetId of selectedMembers) {
				const result = await dbGroup.deleteMember(
					selectedGroupId,
					targetId
				);

				if (result) msg = "*성공적으로 삭제되었습니다*";
			}
			try {
				const result = await client.views.publish({
					user_id: body.user.id,
