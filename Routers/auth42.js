...
router.use(session({ resave: false, saveUninitialized: false, secret: process.env.SECRET_KEY }));
router.use(passport.initialize());
router.use(passport.session());
router.get("/", (req, res, next) => {
	const passportOptions = { state: req.query.id };
...
router.get('/return', passport.authenticate('42', passportOptions), async (req, res) => {
	const slackId = decrypt(req.params.state);
