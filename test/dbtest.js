...
async function test2() {
	...
	const hyeyukim = User.create({intraId: "hyeyukim", slackId: "BBBBBBBB"});
	const sanghwal = User.create({intraId: "sanghwal", slackId: "CCCCCCCC"});
	...
	await groupFuncs.insertGroup(hyeyukim.intraId, "hyeyukim group");
	...
	const yonshinGroup2 = await Group.findOne({ where: {intraId: "sanghwal"}});
	...
	assert.equal((await Group.findOne({ where: {intraId: "sanghwal"}})).name, "yonshin group 2");
	...
	assert.equal(
		(await groupFuncs.getSelectedGroupId(yonshin.intraId)).name,
		"yonshin group 1"
	);
	assert.equal(
		(await groupFuncs.getSelectedGroupId(hyeyukim.intraId)).name,
		"hyeyukim group 1"
	);
